...
export async function seedDatabase(opts?: SeedOptions) {
  const adminUsername = opts?.username || process.env.INITIAL_ADMIN_USER || 'admin';
  const adminPassword = opts?.password || process.env.INITIAL_ADMIN_PASSWORD || 'admin123';
  const adminEmail = opts?.email || process.env.INITIAL_ADMIN_EMAIL || 'admin@proxpanel.io';
  ...
  try {
    ...
    if (!existingAdmin) {
      ...
      await prisma.user.update({
        where: { username: adminUsername },
        data: { password: hashedPassword, email: adminEmail, role: 'SUPER_ADMIN' },
      });
      console.log(`[Seed] Updated SUPER_ADMIN user: ${adminUsername}`);
    }
  } catch (error: any) {
    console.error('[Seed] Error configuring admin user:', error.message);
    throw error;
  }
  ...
  try {
    ...
    console.warn('[Seed] Warning: Failed to seed plans:', error.message);
  }
  ...
  try {
    ...
    console.warn('[Seed] Warning: Failed to seed system settings:', error.message);
  }
  ...
  try {
    ...
    console.warn('[Seed] Warning: Failed to seed translations:', error.message);
  }
}
...
async function cli() {
  ...
  await seedDatabase({ username, password, email });
}
