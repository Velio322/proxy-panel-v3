...
export function statusColor(s: string): string {
  return s === 'ONLINE' ? 'text-emerald-400' : s === 'ERROR' ? 'text-rose-400' : 'text-fg-subtle';
}

export function statusBg(s: string): string {
  return s === 'ONLINE' ? 'bg-emerald-500/10 border-emerald-500/20' : s === 'ERROR' ? 'bg-rose-500/10 border-rose-500/20' : 'bg-surface border-border';
}
